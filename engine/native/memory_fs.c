...
static int busy(memory_file *file, int exclusive) {
    for (int i = 0; i < FILE_COUNT; ++i)
        if (handles[i].stream && handles[i].file == file && (exclusive || handles[i].writable)) {
            errno = EBUSY;
            return 1;
        }
    return 0;
...
FILE *memory_open(const char *path, const char *mode) {
    ...
    if (writable && reserve(file, MEMORY_FILE_CAPACITY))
        return NULL;
    ...
    if (mode[0] == 'w')
        file->size = 0;
    if (mode[0] == 'a')
        fseek(stream, file->size, SEEK_SET);
...
int __wrap_rename(const char *old_path, const char *new_path) {
    ...
    if (destination && busy(destination, 1))
        return -1;
